...
const checkMobileNavigation = () => {
  ...
  if (mobileNav && isMobile) {
    ...
    navButtons.forEach((button, index) => {
      ...
      console.log(
        `  ${index + 1}. ${label} - ${isSelected ? 'Active' : 'Inactive'}`
      );
    });
...
const runMobileNavigationTests = () => {
  ...
  console.log(
    ...
    mobileNavWorking ? 'Working' : 'Needs attention'
  );
...
console.log(
  '📱 Switch to mobile view in DevTools and reload to test mobile navigation'
);
