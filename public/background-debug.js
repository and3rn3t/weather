...
console.log = function(...args) {
  if (args[0] && args[0].includes && args[0].includes('Theme background changing')) {
    themeChangeCount++;
...
if (typeof React !== 'undefined') {
  React.useEffect = function(effect, deps) {
    if (deps && deps.some && deps.some(dep => 
        typeof dep === 'string' && 
        (dep.includes('rgb') || dep.includes('#') || dep.includes('background')))) {
      themeRenderCount++;
...
console.log('✅ Debug script loaded. Click around and watch for unexpected background changes.');
console.log('🎯 Expected: Background should ONLY change when clicking the theme toggle (☀️/🌙)');
console.log('🚨 Issue: If background changes on ANY click, there\'s a bug to fix.');
