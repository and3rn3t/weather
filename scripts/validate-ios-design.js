...
class iOSDesignValidator {
  constructor() {
    ...
    this.standards = {
      colors: {
        primary: ['#007AFF', '#5856D6', '#AF52DE', '#FF2D92', '#FF3B30', '#FF9500', '#FFCC00', '#34C759', '#00C7BE', '#30B0C7'],
      ...
      typography: {
        ...
        families: ['SF Pro', 'SF Pro Display', 'SF Compact', 'system-ui', '-apple-system']
      },
      spacing: {
  ...
  async validateColorSystem() {
    ...
    try {
      ...
      if (!themeConfig.includes('lightTheme') || !themeConfig.includes('darkTheme')) {
      ...
      const semanticColors = ['primary', 'secondary', 'background', 'surface', 'error', 'warning', 'success'];
      const missingSemanticColors = semanticColors.filter(color => !themeConfig.includes(color));
      
      if (missingSemanticColors.length > 0) {
        this.violations.push(`Missing semantic colors: ${missingSemanticColors.join(', ')}`);
      } else {
        this.passed.push('Semantic color system implemented');
      }
      
      // Check for iOS-standard color values
      const hasSystemColors = this.standards.colors.primary.some(color => themeConfig.includes(color));
      if (!hasSystemColors) {
        this.warnings.push('Consider using iOS system colors for better platform consistency');
      } else {
  ...
  async validateTypography() {
    ...
    try {
      ...
      const hasSFPro = typographySystem.includes('SF Pro') || typographySystem.includes('-apple-system');
      if (!hasSFPro) {
        this.violations.push('Missing iOS-standard font families (SF Pro, -apple-system)');
      } else {
        this.passed.push('iOS font families implemented');
      }
      
      // Check for responsive typography
      const hasFluidTypography = typographySystem.includes('clamp(') || typographySystem.includes('vw');
      if (!hasFluidTypography) {
        this.warnings.push('Consider implementing fluid typography for better responsiveness');
      } else {
      ...
      const hasSemanticStyles = textStyles.some(style => typographySystem.includes(style));
      
      if (!hasSemanticStyles) {
        this.warnings.push('Consider implementing semantic text styles (headline, title, body, etc.)');
      } else {
  ...
  async validateSpacing() {
    ...
    try {
      ...
      if (!follows8pxGrid) {
        ...
        this.warnings.push(`Spacing values not following 4px grid: ${violations.join(', ')}px`);
      } else {
      ...
      if (uniqueSpacing.length > 20) {
        this.warnings.push(`Too many spacing values (${uniqueSpacing.length}). Consider standardizing.`);
      } else {
  ...
  async validateAccessibility() {
    ...
    try {
      ...
      for (const file of tsxFiles) {
        ...
        const buttons = (content.match(/<button|<input|<select|<textarea/g) || []).length;
        const customInteractive = (content.match(/role="button"|onClick=/g) || []).length;
      ...
      const accessibilityRatio = totalInteractiveElements > 0 ? accessibilityScore / totalInteractiveElements : 1;
      
      if (accessibilityRatio < 0.5) {
        this.violations.push(`Low accessibility compliance: ${Math.round(accessibilityRatio * 100)}% of interactive elements have accessibility attributes`);
      } else if (accessibilityRatio < 0.8) {
        this.warnings.push(`Moderate accessibility compliance: ${Math.round(accessibilityRatio * 100)}% of interactive elements have accessibility attributes`);
      } else {
        this.passed.push(`High accessibility compliance: ${Math.round(accessibilityRatio * 100)}% of interactive elements have accessibility attributes`);
      }
      
      // Check for semantic HTML
      const componentFiles = await glob('src/components/**/*.{ts,tsx}', { cwd: projectRoot });
      let semanticScore = 0;
      
      for (const file of componentFiles) {
        const content = readFileSync(join(projectRoot, file), 'utf8');
        
        const semanticElements = (content.match(/<(nav|main|section|article|aside|header|footer|button)/g) || []).length;
      ...
      if (avgSemanticScore < 0.3) {
        this.warnings.push('Low semantic HTML usage. Consider using more semantic elements.');
      } else {
  ...
  async validateAnimations() {
    ...
    try {
      ...
      const durationRegex = /(?:transition-duration|animation-duration):\s*([0-9.]+)s/g;
      const durations = [];
      let match;
      ...
      const nonStandardDurations = durations.filter(d => !this.standards.animation.durations.includes(d));
      
      if (nonStandardDurations.length > 0) {
        this.warnings.push(`Non-standard animation durations detected: ${nonStandardDurations.join('s, ')}s`);
      ...
      const hasProperEasing = easings.some(easing => 
        this.standards.animation.easings.some(standard => easing.includes(standard))
      );
      ...
      const hasReducedMotion = animationCSS.includes('@media (prefers-reduced-motion');
      if (!hasReducedMotion) {
        this.violations.push('Missing reduced motion support (@media (prefers-reduced-motion))');
      } else {
  ...
  async validateComponentStructure() {
    ...
    try {
      ...
      for (const file of componentFiles) {
        ...
        const hasMatchingComponentName = content.includes(`function ${fileName}`) || 
      ...
      const structureScore = properlyStructuredComponents / componentFiles.length;
      
      if (structureScore < 0.7) {
        this.warnings.push(`${Math.round(structureScore * 100)}% of components follow proper structure conventions`);
      } else {
        this.passed.push(`${Math.round(structureScore * 100)}% of components follow proper structure conventions`);
      }
      
    } catch (error) {
      this.violations.push(`Component structure validation failed: ${error.message}`);
    }
  }
  ...
  printResults() {
    ...
    console.log(`🎯 Passed: ${this.passed.length}, Warnings: ${this.warnings.length}, Violations: ${this.violations.length}\n`);
