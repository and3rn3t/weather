...
class QuickSetup {
  ...
  async checkPrerequisites() {
    ...
    await this.logStep('Checking Node.js version', async () => {
      ...
      if (versionNumber < 20) {
        throw new Error(
          `Node.js ${version} is too old. Please upgrade to v20+`
        );
      }
  ...
  async runInitialTests() {
    ...
    await this.logStep('Running TypeScript check', async () => {
      try {
        ...
        console.log(
          '   TypeScript: Some warnings found (see previous build logs)'
        );
      }
    });
    ...
    try {
      ...
      if (this.errors.length === 0) {
        ...
        if (process.argv.includes('--start')) {
          ...
          console.log(
            'Run "npm run dev" to start the development server when ready.'
          );
        }
      } else {
