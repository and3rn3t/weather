...
class PerformanceMonitor {
  ...
  async analyzeBundleSize() {
    ...
    if (totalSizeKB > this.thresholds.bundleSize) {
      console.log(`⚠️ Bundle size ${totalSizeKB.toFixed(1)}KB exceeds threshold ${this.thresholds.bundleSize}KB`);
    } else {
      console.log(`✅ Bundle size ${totalSizeKB.toFixed(1)}KB within threshold`);
    }
  ...
  async analyzeDependencies() {
    ...
    let unusedDeps = [];
    try {
      const { stdout } = await execAsync('npx depcheck --json', { cwd: projectRoot });
    ...
    if (depMetrics.totalDependencies > this.thresholds.dependencies) {
      ...
      console.log(`✅ Dependency count within threshold: ${depMetrics.totalDependencies}`);
    }
  ...
  async findHeaviestDependencies(deps) {
    ...
    for (const dep of deps) {
      try {
        ...
        if (existsSync(depPath)) {
          ...
          heaviest.push({ name: dep, sizeMB: (size / (1024 * 1024)).toFixed(2) });
        }
      } catch {
    ...
    return heaviest.sort((a, b) => parseFloat(b.sizeMB) - parseFloat(a.sizeMB)).slice(0, 5);
  }
  ...
  async measureBuildTime() {
    ...
    try {
      ...
      if (buildTime > this.thresholds.buildTime) {
        console.log(`⚠️ Build time ${buildTime.toFixed(1)}s exceeds threshold ${this.thresholds.buildTime}s`);
      } else {
    ...
    try {
      ...
      for (const [metric, score] of Object.entries(scores)) {
        if (score < this.thresholds.lighthouse[metric]) {
          failedThresholds.push(`${metric}: ${score} < ${this.thresholds.lighthouse[metric]}`);
        }
      }
      
      if (failedThresholds.length > 0) {
        console.log(`⚠️ Lighthouse scores below threshold: ${failedThresholds.join(', ')}`);
      } else {
  ...
  async checkRuntimePerformance() {
    ...
    const jsFiles = srcFiles.filter(f => f.endsWith('.ts') || f.endsWith('.tsx'));
    ...
    for (const file of jsFiles.slice(0, 20)) { // Check first 20 files
      try {
        ...
        if (content.includes('useEffect(') && content.includes('[]') === false) {
          ...
          if (effectCount > 3) {
            performanceIssues.push(`${file}: Multiple useEffect hooks (${effectCount})`);
          }
        }
        ...
        if (content.includes('console.log') && !file.includes('test')) {
          performanceIssues.push(`${file}: Console.log statements in production code`);
        }
    ...
    if (performanceIssues.length > 0) {
      console.log(`⚠️ Found ${performanceIssues.length} potential runtime performance issues`);
    } else {
  ...
  generatePerformanceReport() {
    ...
    const reportPath = join(projectRoot, `performance-report-${Date.now()}.json`);
  ...
  generateSummary() {
    ...
    if (this.metrics.build.buildTime) {
      if (this.metrics.build.buildTime > this.thresholds.buildTime) {
        ...
        successes.push(`Build time: ${this.metrics.build.buildTime.toFixed(1)}s`);
      }
    }
    
    // Dependencies
    if (this.metrics.dependencies.totalDependencies > this.thresholds.dependencies) {
      issues.push(`Dependencies: ${this.metrics.dependencies.totalDependencies}`);
    } else {
      successes.push(`Dependencies: ${this.metrics.dependencies.totalDependencies}`);
    }
    
    // Lighthouse
    if (this.metrics.lighthouse && !this.metrics.lighthouse.error) {
      const avgScore = Object.values(this.metrics.lighthouse).reduce((a, b) => a + b, 0) / 4;
      if (avgScore < 90) {
  ...
  calculateOverallScore() {
    ...
    if (this.metrics.bundle.totalSize && this.metrics.bundle.totalSize / 1024 > this.thresholds.bundleSize) {
      score -= 15;
    }
  ...
  printResults() {
    ...
    if (this.metrics.bundle.totalSize) {
      const sizeMB = (this.metrics.bundle.totalSize / (1024 * 1024)).toFixed(2);
      console.log(`📦 Bundle Size: ${sizeMB}MB (${this.metrics.bundle.fileCount} files)`);
      console.log(`   - JavaScript: ${(this.metrics.bundle.jsSize / 1024).toFixed(1)}KB`);
      console.log(`   - CSS: ${(this.metrics.bundle.cssSize / 1024).toFixed(1)}KB`);
      console.log(`   - Assets: ${(this.metrics.bundle.assetsSize / 1024).toFixed(1)}KB\n`);
    }
    
    if (this.metrics.build.buildTime) {
      console.log(`⏱️ Build Time: ${this.metrics.build.buildTime.toFixed(1)}s\n`);
    }
    
    if (this.metrics.dependencies.totalDependencies) {
      console.log(`📚 Dependencies: ${this.metrics.dependencies.totalDependencies} total`);
      console.log(`   - Production: ${this.metrics.dependencies.dependencies}`);
      console.log(`   - Development: ${this.metrics.dependencies.devDependencies}`);
...
if (process.argv.includes('--help')) {
  ...
  console.log('  node performance-monitor.js              # Basic performance check');
  console.log('  node performance-monitor.js --lighthouse # Include Lighthouse scores');
  process.exit(0);
}
