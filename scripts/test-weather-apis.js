...
class WeatherAPITester {
  ...
  async testOpenMeteoHourly() {
    const url =
      'https://api.open-meteo.com/v1/forecast?latitude=40.7128&longitude=-74.0060&hourly=temperature_2m,relative_humidity_2m,weather_code&temperature_unit=fahrenheit';
    const response = await fetch(url);
  ...
  async testOpenMeteoDaily() {
    const url =
      'https://api.open-meteo.com/v1/forecast?latitude=40.7128&longitude=-74.0060&daily=temperature_2m_max,temperature_2m_min,weather_code,precipitation_sum&temperature_unit=fahrenheit';
    const response = await fetch(url);
  ...
  async testErrorHandling() {
    ...
    const response = await fetch(
      'https://nominatim.openstreetmap.org/search?q=ThisCityDoesNotExist12345&format=json&limit=1',
      {
        headers: { 'User-Agent': 'WeatherApp-Test/1.0' },
      }
    );
  ...
  async run() {
    ...
    await this.test('OpenMeteo - Basic Current Weather', () =>
      this.testOpenMeteoBasic()
    );
    await this.test('OpenMeteo - Hourly Forecast', () =>
      this.testOpenMeteoHourly()
    );
    await this.test('OpenMeteo - Daily Forecast', () =>
      this.testOpenMeteoDaily()
    );
    await this.test('Nominatim - Basic Geocoding', () =>
      this.testNominatimBasic()
    );
    await this.test('Nominatim - Special Characters', () =>
      this.testNominatimSpecialCharacters()
    );
    await this.test('API Rate Limits', () => this.testAPIRateLimits());
    await this.test('Weather Code Mapping', () =>
      this.testWeatherCodeMapping()
    );
