...
try {
  ...
  files.forEach(file => {
    ...
    if (fs.statSync(filePath).isFile()) {
      ...
      if (sizeKB > BUDGET.largestFile) {
        violations.push(
          `Large file: ${file} (${sizeKB} KB > ${BUDGET.largestFile} KB)`
        );
      }
    }
  });
  ...
  if (totalSizeKB > BUDGET.totalBundle) {
    violations.push(
      `Total bundle too large: ${totalSizeKB} KB > ${BUDGET.totalBundle} KB`
    );
    console.log('❌ FAIL');
  } else {
    console.log('✅ PASS');
  ...
  if (jsSizeKB > BUDGET.jsBundle) {
    violations.push(
      `JS bundle too large: ${jsSizeKB} KB > ${BUDGET.jsBundle} KB`
    );
    console.log('❌ FAIL');
  } else {
    console.log('✅ PASS');
  ...
  if (cssSizeKB > BUDGET.cssBundle) {
    violations.push(
      `CSS bundle too large: ${cssSizeKB} KB > ${BUDGET.cssBundle} KB`
    );
    console.log('❌ FAIL');
  } else {
    console.log('✅ PASS');
  ...
  // Check largest file
  console.log(
    `📄 Largest File: ${largestFileSize} KB / ${BUDGET.largestFile} KB`
  );
