...
class MobileDeploymentAssistant {
  ...
  async checkPrerequisites() {
    ...
    if (nodeResult.success) {
      ...
      if (versionNumber < 20) {
        this.log(
          `Node.js ${version} detected. Recommend upgrading to v20+`,
          'warning'
        );
      } else {
    ...
    // Check Capacitor CLI
    const capacitorResult = await this.runCommand(
      'npx cap --version',
      'Capacitor CLI check'
    );
  ...
  async checkiOSPrerequisites() {
    ...
    if (process.platform === 'darwin') {
      const xcodeResult = await this.runCommand(
        'xcodebuild -version',
        'Xcode version check'
      );
      ...
      // Check iOS Simulator
      const simulatorResult = await this.runCommand(
      ...
      this.log(
        'iOS development requires macOS. Using web preview mode.',
        'warning'
      );
    }
    ...
    if (existsSync(join(projectRoot, 'ios', 'Podfile'))) {
      const podResult = await this.runCommand(
        'pod --version',
        'CocoaPods check'
      );
  ...
  async checkAndroidPrerequisites() {
    ...
    if (process.env.ANDROID_HOME || process.env.ANDROID_SDK_ROOT) {
      ...
      this.log(
        'Android SDK not configured. Set ANDROID_HOME environment variable.',
        'warning'
      );
    }

    // Check Java
    const javaResult = await this.runCommand(
      'java -version',
      'Java version check'
    );
  ...
  async setupCapacitor() {
    ...
    await this.runCommand(
      'npx cap copy',
      'Copying web assets to native projects'
    );
  }
  ...
  async addPlatform() {
    ...
    if (!existsSync(platformPath)) {
      const addResult = await this.runCommand(
        `npx cap add ${this.platform}`,
        `Adding ${this.platform} platform`
      );
  ...
  async syncPlatform() {
    ...
    const syncResult = await this.runCommand(
      `npx cap sync ${this.platform}`,
      `Syncing ${this.platform}`
    );
  ...
  async runOnDevice() {
    ...
    if (this.platform === 'ios') {
      if (process.platform === 'darwin') {
        ...
        this.log(
          'iOS simulator not available on this platform. Opening Xcode project...',
          'info'
        );
        this.log(
          `Please open: ${join(projectRoot, 'ios', 'App.xcworkspace')}`,
          'info'
        );
      }
    } else if (this.platform === 'android') {
      // Check for connected devices
      const devicesResult = await this.runCommand(
        'adb devices',
      ...
      if (devicesResult.success && devicesResult.output.includes('device')) {
        ...
        this.log(
          'No Android devices detected. Opening Android Studio...',
          'info'
        );
        await this.runCommand(
  ...
  async openInIDE() {
    this.log(
      `💻 Opening in ${this.platform === 'ios' ? 'Xcode' : 'Android Studio'}...`
    );
  ...
  async generateSplashScreens() {
    ...
    if (existsSync(splashPath)) {
      ...
      this.log(
        'No splash screen found. Consider adding public/splash.png',
        'warning'
      );
    }
  }
  ...
  generateDeploymentReport() {
    ...
    const reportPath = join(
      projectRoot,
      `deployment-report-${this.platform}-${Date.now()}.json`
    );
  ...
  async run() {
    console.log(
      `📱 Mobile Deployment Assistant - ${this.platform.toUpperCase()} ${this.buildType.toUpperCase()}\n`
    );
