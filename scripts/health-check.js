...
class HealthChecker {
    ...
    try {
      ...
      this.results.dependencies = {
        ...
        details: [
          ...
          outdatedCount > 0
            ? `Consider updating: ${Object.keys(outdated).slice(0, 3).join(', ')}`
  ...
  async checkAPIs() {
    ...
    for (const api of apis) {
      try {
        ...
        if (response.ok) {
          results.push(
            `✅ ${api.name}: ${response.status} (${Math.round(performance.now())}ms)`
          );
        } else {
  ...
  async checkBuild() {
    ...
    try {
      ...
      const { stdout: viteCheck } = await execAsync('npx vite --version', {
        cwd: projectRoot,
      });
      ...
      this.results.build = {
        ...
        details: [
          ...
          hasErrors
            ? `TypeScript warnings: ${tscErrors.split('\n').length} lines`
  ...
  printResults() {
    ...
    for (const [category, result] of Object.entries(this.results)) {
      console.log(
        `${getStatusIcon(result.status)} ${category.toUpperCase()}: ${result.status.toUpperCase()}`
      );
      result.details.forEach(detail => console.log(`   ${detail}`));
      console.log('');
    }

    const overallHealth = Object.values(this.results).every(
      r => r.status === 'healthy'
    )
      ? 'HEALTHY'
      : Object.values(this.results).some(r => r.status === 'error')
        ? 'NEEDS ATTENTION'
        : 'FAIR';
