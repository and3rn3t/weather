...
if ($totalDeps -gt 150) {
    Write-Host "Warning: High dependency count may impact build time" -ForegroundColor Yellow
}
...
if (Test-Path "dist") {
    ...
    if ($buildSeconds -lt $targetTime) {
        ...
        Write-Host "Warning: Build time above target (${buildSeconds}s > ${targetTime}s)" -ForegroundColor Yellow
    }
    
    if ($distSizeMB -lt $targetSize) {
        Write-Success "Build size within target (< ${targetSize}MB)"
    } else {
        Write-Host "Warning: Build size above target (${distSizeMB}MB > ${targetSize}MB)" -ForegroundColor Yellow
    }
}
