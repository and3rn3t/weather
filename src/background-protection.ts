...
document.addEventListener('click', function(event) {
  ...
  const isThemeToggle = target.closest('.theme-toggle-btn') || 
                       target.classList.contains('theme-toggle-btn') ||
                       (target.textContent && (target.textContent.includes('☀️') || target.textContent.includes('🌙')));
  
  if (!isThemeToggle) {
    // This is NOT a theme toggle click - prevent any theme changes
    const now = Date.now();
    if (now - lastThemeToggleTime > 1000) { // Only if not within 1 second of legitimate toggle
      // Block theme context from running
      console.log('🚫 Blocking potential accidental theme change from click on:', target);
