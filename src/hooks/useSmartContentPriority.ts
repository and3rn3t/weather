...
// import { useMobileOptimizations } from '../utils/useMobileOptimizations';
...
export const useSmartContentPriority = (weatherContext: WeatherContext) => {
  ...
  const calculatePriorities = useCallback((): ContentPriority[] => {
    ...
    const isSlowConnection = optimizedSettings.imageQuality === 'low';
  ...
  }, [deviceInfo, optimizedSettings, weatherContext, userPreferences]);
  ...
  const getLayoutConfig = useCallback(() => {
    ...
    return {
      ...
      cardSpacing: isCompact ? 'tight' : 'comfortable',
      textDensity: isCompact ? 'dense' : 'normal',
    };
...
export default useSmartContentPriority;
