...
const getEnhancedScreenInfo = (): EnhancedScreenInfo => {
  ...
  const hasNotch = typeof window !== 'undefined' && 
    'CSS' in window && 
    window.CSS && 
...
const getEnhancedTypography = (screenInfo: EnhancedScreenInfo): EnhancedTypography => {
  ...
  return {
    ...
    medium: clampFont(baseFontSize + 1, baseFontSize * scaleRatio, baseFontSize + 4),
    large: clampFont(baseFontSize + 2, baseFontSize * Math.pow(scaleRatio, 2), baseFontSize + 6),
    xlarge: clampFont(baseFontSize + 4, baseFontSize * Math.pow(scaleRatio, 3), baseFontSize + 10),
    xxlarge: clampFont(baseFontSize + 6, baseFontSize * Math.pow(scaleRatio, 4), baseFontSize + 14),
    hero: clampFont(baseFontSize + 12, baseFontSize * Math.pow(scaleRatio, 5), baseFontSize + 24),
    display: clampFont(baseFontSize + 16, baseFontSize * Math.pow(scaleRatio, 6), baseFontSize + 32)
  };
};
...
const getEnhancedSpacing = (screenInfo: EnhancedScreenInfo): EnhancedSpacing => {
  ...
  const clampSpacing = (min: number, preferred: number, max: number): string => {
...
const getEnhancedTouchTargets = (screenInfo: EnhancedScreenInfo): EnhancedTouchTargets => {
...
const getEnhancedBorderRadius = (screenInfo: EnhancedScreenInfo): EnhancedBorderRadius => {
...
export const useEnhancedMobileScreen = () => {
  const [screenInfo, setScreenInfo] = useState<EnhancedScreenInfo>(getEnhancedScreenInfo);
  ...
  const typography = useMemo(() => getEnhancedTypography(screenInfo), [screenInfo]);
  const spacing = useMemo(() => getEnhancedSpacing(screenInfo), [screenInfo]);
  const touchTargets = useMemo(() => getEnhancedTouchTargets(screenInfo), [screenInfo]);
  const borderRadius = useMemo(() => getEnhancedBorderRadius(screenInfo), [screenInfo]);
  
  // Utility functions
  const getResponsiveFontSize = useCallback((size: keyof EnhancedTypography) => {
    return typography[size];
  }, [typography]);
  
  const getResponsiveSpacing = useCallback((size: keyof EnhancedSpacing) => {
    return spacing[size];
  }, [spacing]);
  
  const getOptimalTouchTarget = useCallback((priority: 'minimum' | 'comfortable' | 'large' | 'gesture' = 'comfortable') => {
    return touchTargets[priority];
  }, [touchTargets]);
  
  const getAdaptiveBorderRadius = useCallback((size: keyof EnhancedBorderRadius) => {
