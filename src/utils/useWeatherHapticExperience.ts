...
const WEATHER_HAPTIC_MAPPING: Record<number, {
  basePattern: string;
  intensityRange: [number, number];
  description: string;
}> = {
  // Clear conditions
  0: { basePattern: 'gentle-pulse', intensityRange: [0.3, 0.5], description: 'Clear sunny warmth' },
  1: { basePattern: 'soft-wave', intensityRange: [0.4, 0.6], description: 'Mostly clear breeze' },
  
  // Cloudy conditions  
  2: { basePattern: 'rolling-cloud', intensityRange: [0.5, 0.7], description: 'Partly cloudy drift' },
  3: { basePattern: 'heavy-blanket', intensityRange: [0.6, 0.8], description: 'Overcast weight' },
  
  // Fog
  45: { basePattern: 'misty-softness', intensityRange: [0.2, 0.4], description: 'Foggy mystery' },
  48: { basePattern: 'crystalline-fog', intensityRange: [0.3, 0.5], description: 'Rime fog chill' },
  
  // Drizzle
  51: { basePattern: 'tiny-droplets', intensityRange: [0.3, 0.5], description: 'Light drizzle patter' },
  53: { basePattern: 'steady-drops', intensityRange: [0.4, 0.6], description: 'Moderate drizzle' },
  55: { basePattern: 'dense-mist', intensityRange: [0.5, 0.7], description: 'Dense drizzle shroud' },
  
  // Rain
  61: { basePattern: 'gentle-rain', intensityRange: [0.5, 0.7], description: 'Light rain melody' },
  63: { basePattern: 'steady-rain', intensityRange: [0.6, 0.8], description: 'Moderate rain rhythm' },
  65: { basePattern: 'heavy-downpour', intensityRange: [0.8, 1.0], description: 'Heavy rain power' },
  
  // Snow
  71: { basePattern: 'falling-snow', intensityRange: [0.2, 0.4], description: 'Light snow flutter' },
  73: { basePattern: 'thick-snow', intensityRange: [0.4, 0.6], description: 'Moderate snow blanket' },
  75: { basePattern: 'blizzard', intensityRange: [0.7, 0.9], description: 'Heavy snow storm' },
  
  // Thunderstorm
  95: { basePattern: 'thunder-strike', intensityRange: [0.9, 1.0], description: 'Thunderstorm power' }
};
...
export const useWeatherHapticExperience = (config: WeatherHapticConfig = {}) => {
  const {
  ...
  const createWeatherExperience = useCallback((condition: WeatherCondition): HapticWeatherExperience => {
    ...
    let intensity = (mapping.intensityRange[0] + mapping.intensityRange[1]) / 2;
  ...
  const executeWeatherHaptic = useCallback(async (pattern: string, intensity: number = 1.0) => {
    ...
    const scaledPattern = patternData.map(duration => Math.round(duration * intensity));
  ...
  const triggerWeatherHaptic = useCallback(async (condition: WeatherCondition) => {
    ...
    logWeatherHaptic(`${experience.description} (intensity: ${experience.intensity.toFixed(2)})`);
  ...
  const triggerTemperatureChange = useCallback(async (oldTemp: number, newTemp: number) => {
    ...
    if (newTemp > oldTemp) {
      await executeWeatherHaptic('temperature-rising', Math.min(1.0, tempDiff / 10));
    } else {
      await executeWeatherHaptic('temperature-falling', Math.min(1.0, tempDiff / 10));
    }
  }, [enableProgressiveFeedback, executeWeatherHaptic]);

  // Pressure change haptic
  const triggerPressureChange = useCallback(async (pressureChange: number) => {
    if (!enableProgressiveFeedback) return;
  ...
  const triggerWindGust = useCallback(async (windSpeed: number) => {
    if (!enableAtmosphericPatterns) return;
  ...
  const triggerWeatherTransition = useCallback(async (fromCondition: WeatherCondition, toCondition: WeatherCondition) => {
    if (!enableWeatherContext) return;
    ...
    const blendedIntensity = (fromExperience.intensity + toExperience.intensity) / 2;
    ...
    logWeatherTransition(`${fromExperience.description} → ${toExperience.description}`);
    
    try {
      if (navigator.vibrate) {
        const scaledPattern = transitionPattern.map(duration => Math.round(duration * blendedIntensity));
        navigator.vibrate(scaledPattern);
      }
    } catch (error) {
  ...
  const triggerWeatherRefresh = useCallback(async (condition?: WeatherCondition) => {
