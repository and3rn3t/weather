 ...
 */
export const executeVibration = (pattern: number[] | keyof typeof HAPTIC_PATTERNS, intensity: number = 1.0): boolean => {
  try {
    // Get pattern array
    ...
    const scaledPattern = patternArray.map(duration => Math.round(duration * intensity));
 ...
 */
export const executeWeatherVibration = (weatherCode: number, intensity: number = 1.0): boolean => {
