...
export const useHapticFeedback = (config: HapticConfig = {}) => {
  ...
  const executeVibration = useCallback((pattern: number | number[]) => {
  ...
  const triggerHaptic = useCallback((pattern: HapticPatternType | number | number[]) => {
  ...
  const selection = useCallback(() => triggerHaptic(HapticPattern.SELECTION), [triggerHaptic]);
  const refresh = useCallback(() => triggerHaptic(HapticPattern.REFRESH), [triggerHaptic]);
  const navigation = useCallback(() => triggerHaptic(HapticPattern.NAVIGATION), [triggerHaptic]);
  const longPress = useCallback(() => triggerHaptic(HapticPattern.LONG_PRESS), [triggerHaptic]);

  // ============================================================================
  // AUDIO FALLBACK (Optional)
  // ============================================================================

  const playAudioFeedback = useCallback((type: 'click' | 'success' | 'error' = 'click') => {
