...
const SimpleWeatherApp: React.FC = () => {
  ...
  const [dailyForecast, setDailyForecast] = useState<Array<{
    date: string;
    weatherCode: number;
    tempMax: number;
    tempMin: number;
  ...
  const getWeather = async () => {
    ...
    try {
      ...
      const hourlyData = weatherData.hourly.time.slice(0, 24).map((time: string, index: number) => ({
      ...
      const dailyData = weatherData.daily.time.map((date: string, index: number) => ({
  ...
  const fetchWeatherForLocation = async (lat: number, lon: number) => {
    ...
    try {
      ...
      const currentWeather: WeatherData = {
        ...
        weather: [{ description: getWeatherDescription(weatherData.current.weathercode) }],
        wind: {
          speed: Math.round(weatherData.current.windspeed_10m || 0),
          deg: weatherData.current.winddirection_10m || 0
        },
        weatherCode: weatherData.current.weathercode,
        uv_index: weatherData.current.uv_index || 0,
        visibility: Math.round((weatherData.current.visibility || 10000) / 1609.34)
      };
